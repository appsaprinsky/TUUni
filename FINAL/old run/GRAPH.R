...
Create4graphs <- function(data_MAIN, final_column_num, final_column_name_str){
  ...
  my_plot1 <-ggplot(BRMSEA_TM, aes_string(x = "PersonSize", y= final_column_name_str, color = "as.factor( ModelTorF)")) +
    ...
    labs(x = "PersonSize", y = final_column_name_str, color = "ModelTorF", title = paste(final_column_name_str, " ", sep = " ") ) +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5))
...
Create5graphs <- function(data_MAIN, final_column_num, final_column_name_str){
  ...
  for (column in columns_to_plot) {  
    ...
    my_plot1 <-ggplot(BRMSEA_TM, aes_string(x = "log(PersonSize)", y= column, color = "as.factor( TimePoint)")) +
      geom_line() +
      facet_wrap(~ ModelTorF, scales = "free_y", nrow = 3, ncol = 2) +
      labs(x = "log(sample size)", y = column, color = "TimePoint", title = paste(column, "", sep = " ") ) +
      theme_minimal() +
      theme(plot.title = element_text(hjust = 0.5)) +
      coord_cartesian(ylim = c(min(BRMSEA_TM[[column]]), max(BRMSEA_TM[[column]])))
...
# data$PersonSize <- log(data$PersonSize)





Create6graphs <- function(data_MAIN, final_column_num, final_column_name_str){
  ...
  for (column in columns_to_plot) {  
    ...
    my_plot1 <-ggplot(BRMSEA_TM, aes_string(x = "PersonSize", y= column, color = "as.factor( TimePoint)")) +
      geom_line() +
      facet_wrap(~ ModelTorF, scales = "free_y", nrow = 3, ncol = 2) +
      labs(x = "sample size", y = column, color = "TimePoint", title = paste(column, "", sep = " ") ) +
      theme_minimal() +
      theme(plot.title = element_text(hjust = 0.5)) +
      scale_x_log10()+
      coord_cartesian(ylim = c(min(BRMSEA_TM[[column]]), max(BRMSEA_TM[[column]])))
