...
# updated version
# HB: The covariance matrix is pos. def. if the dimension of 
# the cov+1 >= N (e.g., 5 time points x 6 variables=30, N_min=31)
person_size_SIMULATE <- c(31, 61, 91) # N
#(person_size_SIMULATE-1)/6
# not all conditions are identified
time_point_SIMULATE <- c(5, 10, 15) # Nt
# N:     31 61 91
# Nt: 5  ok ok ok
#    10  x  ok ok
#    15  x  x  ok
run_Samples_SIMULATE <- 1 ############################ 100
model_TRUE_MISS <- c(0, 0.3) 
...
for (model_TRUE_MISS in model_TRUE_MISS_SIMULATE){
  for (person_size in person_size_SIMULATE) {
    for (time_point in time_point_SIMULATE) {
      ...
      ######################################
      # check for now
      #model_TRUE_MISS <- model_TRUE_MISS_SIMULATE[1]
      #person_size <- person_size_SIMULATE[3]
      #time_point <- time_point_SIMULATE[3]
      ######################################
      ...
      for (SAMPLING in 1:run_Samples_SIMULATE){ ### All Samples run !!!!!!!!!!!!!!!!!
        ...
        if(is_positive_def==TRUE){
          # HB: You are fitting the model to the same data set twice. you need to generate new data each time
          # I changed the loop
          #for (SAMPLING in 1:run_Samples_SIMULATE){ ### All Samples run !!!!!!!!!!!!!!!!!
          # this needs to be new data for each run
