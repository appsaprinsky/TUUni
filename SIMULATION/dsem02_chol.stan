...
parameters {
  ...
  //corr_matrix[2] phi;
  cholesky_factor_corr[2] L1;    //cholesky matrix L1
...
transformed parameters{
  ...
  for(j in 2:Nt){
    ...
    eta[j,,] = mueta[j,,] + zi[j,,]*L1';
  }

  //for(j in 1:Nt){
    //eta[j,,] = mueta[j,,] + zi*diag_pre_multiply(phi1,L1)';
  //  eta[j,,] = mueta[j,,] + zi[j,,]*L1';
  //}
}
...
model {
  ...
  for(j in 1:Nt){
    ...
    for (i in 1:N) {
      ...
      //eta[j,i,1:2] ~ multi_normal_cholesky(mueta[j,i,1:2], L1);
  ...
  sigmaeps ~ cauchy(0,5);
  //phi ~ lkj_corr(1);
  L1 ~ lkj_corr_cholesky(1); //cholesky corr matrix
  
}

generated quantities {
  matrix[2,2] phi; //cov(eta)
  ...
  //phi = diag_pre_multiply(phi0,L1)*diag_pre_multiply(phi0,L1)'; //for covariance matrix with SD phi0
  phi = L1*L1'; //for correlation matrix (like here)
