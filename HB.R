  ...
  for (i in 1:6) {
    for (j in 1:6) {
      ...
      for (n in 1:N) {
        int qwer1 = (i > 3) ? 2 : 1;
        int qwer2 = (j > 3) ? 2 : 1;
        cov_x[i, j] += (ly[i]*eta[n, qwer1] - x[n, i]) * (ly[j]*eta[n, qwer2] - x[n, j]); 
      }
      cov_x[i, j] /= N;
...
generated quantities {
  ...
  vector[N] log_lik_sat_stan;
  
  matrix[6, 6] curr_cov_ll;
  // vector[N] log_lik_WISHART;
  // vector[N] log_lik_WISHART_saturated;
  
  vector[6] x_together_ml;
  vector[6] mu_together_ml;  
  vector[N] log_lik_MN;
  vector[N] log_lik_MN_saturated;
  ...
  for (i in 1:N) {
    ...
    log_lik_sat[i] = 0;  
    log_lik_sat[i] += normal_lpdf(x[i,1] | ly[1]*eta[i, 1], cov_x_DATA[1,1]);
    log_lik_sat[i] += normal_lpdf(x[i,2] | ly[2]*eta[i, 1], cov_x_DATA[2,2]);
    log_lik_sat[i] += normal_lpdf(x[i,3] | ly[3]*eta[i, 1], cov_x_DATA[3,3]);
    log_lik_sat[i] += normal_lpdf(x[i,4] | ly[4]*eta[i, 2], cov_x_DATA[4,4]);
    log_lik_sat[i] += normal_lpdf(x[i,5] | ly[5]*eta[i, 2], cov_x_DATA[5,5]);
    log_lik_sat[i] += normal_lpdf(x[i,6] | ly[6]*eta[i, 2], cov_x_DATA[6,6]);
    
    log_lik_sat_stan[i] =  -log_lik[i]+ log_lik_sat[i];
    
    for (aaa in 1:6) {
      for (sss in 1:6) {
        if (aaa == sss) {
          curr_cov_ll[aaa, sss] = epsilon[aaa];
        } else {
          curr_cov_ll[aaa, sss] = 0;  // curr_cov_ll[aaa, sss] = cov_x_DATA[aaa, sss]; 
        }
      }
    }
    //log_lik_WISHART[i] = wishart_lpdf(cov_x_DATA | 6, curr_cov_ll);  
    //log_lik_WISHART_saturated[i] = wishart_lpdf(curr_cov_ll | 6, curr_cov_ll); 
    ...
    log_lik_MN[i] = multi_normal_lpdf(x_together_ml | mu_together_ml, curr_cov_ll); 
    log_lik_MN_saturated[i] = multi_normal_lpdf(x_together_ml | mu_together_ml, cov_x);
    log_lik_sat[i] = 0;
    log_lik[i] = multi_normal_lpdf(x_together_ml | mu_together_ml, curr_cov_ll); 
...
colMeans(x)
# log_lik_WISHART[i] = wishart_lpdf(to_vector(cov_x)| 5, curr_cov_ll);    
# // log_lik_WISHART_saturated[i] = wishart_lpdf(5 * cov_x |  5, cov_x);
...
posterior_samples$log_lik_WISHART
posterior_samples$curr_cov_ll[,,6]
posterior_samples$cov_x_generated[1,,]
posterior_samples$log_lik_MN
posterior_samples$log_lik_MN_saturated
...
ddd2 <- data.frame(posterior_samples$log_lik_WISHART)
sum(ddd2[1,])
rowSums (ddd2)


ddd3 <- data.frame(posterior_samples$log_lik_WISHART_saturated)
sum(ddd3[1,])
rowSums (ddd3)

ddd4 <- data.frame(posterior_samples$log_lik_MN_saturated)
sum(ddd4[1,])
rowSums (ddd4)


posterior_samples$log_lik_sat_stan
...
my_array1 <- Create2DimMatrix(posterior_samples$log_lik_MN, posterior_samples$log_lik_MN_saturated, 1000)
